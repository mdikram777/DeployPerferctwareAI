...
def load_models():
    ...
    # Use Groq API for reliable AI responses
    try:
        ...
        print("✅ AI model loaded successfully with Groq")
...
# qa_chain will be created in main() function
            ...
            try:
                ...
                if qa_chain is not None:
                    try:
                        ...
                        # Generate response using multiple fallback methods
                        try:
                            ...
                            response = None
                            try:
                                # Method 1: Direct chain invocation
                                if qa_chain:
                                    response = qa_chain.invoke({
                                        ...
                                        "customers": ", ".join(COMPANY_INFO['target_customers']),
                                        "values": ", ".join(COMPANY_INFO['values']),
                                        "image_context": " | ".join(image_contexts[:3]),
                                        "history": conversation_context
                                    })
                                    print(f"✅ AI response generated via chain")
                                else:
                                    raise Exception("QA chain not available")
                            except Exception as chain_e:
                                print(f"⚠️ Chain method failed: {chain_e}")
                                try:
                                    # Method 2: Direct LLM invocation
                                    response = llm.invoke(formatted_prompt)
                                    print(f"✅ AI response generated via direct LLM")
                                except Exception as direct_e:
                                    print(f"⚠️ Direct LLM method failed: {direct_e}")
